...
```
crates/shared/       — MarketSnapshot model, technical indicators (EMA, RSI, MACD, ADX, BB, Squeeze, ATR, +25 more)
crates/engine/       — Binary: Hyperliquid/Bitget WS ingestion, indicator pipeline, Axum web server, SQLite telemetry
...
`config.toml` at workspace root controls indicator lookback windows, candle duration, and exchange endpoints. Parsed at startup by `main.rs`. If missing, the engine panics.
