...
| Trade Automation Engine (TAE) | `portfolio-supervisor` | Setup executor (4-TF top-setup aggregation, zone-midpoint geometry, lifecycle state machine — **Lifecycle & Adoption Layer (L2)**, invalidation), unified execution engine + `ExecutionBackend` (`PaperSimulation` default; `LiveBroker` + `BitgetLiveBroker` live), **v8.2 allocation sizing (`allocation_pct` 1–100%, per-instance override, Σ ≤ 100%, ≤100 instances)**, STOP flatten; live `TradeAutomationDashboard` (`/api/instances/:id/automation`) | Implemented (paper default; live dispatch Hyperliquid + Bitget) |
