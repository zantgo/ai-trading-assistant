    ...
    onMount(() => {
        ...
        if (onScreenshotReady) {
            onScreenshotReady(() => {
                ...
                link.download = `${pairKey}_${timeframe}s_hv.png`;
        ...
        (async () => {
            ...
            try {
                ...
                if (ih && ih.hv && ih.hv.length > 0) {
                    ...
                    if (d.length > 0) { hvSeries.setData(d); chart.timeScale().fitContent(); }
                }
            } catch (err) {
                console.error('Error bootstrapping HV history:', err);
            }
        })();
