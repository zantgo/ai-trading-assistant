<script lang="ts">
    ...
    onMount(() => {
        ...
        series = chart.addSeries(LineSeries, { color: '#81c784', lineWidth: 1, priceLineVisible: false });
        series.createPriceLine({ price: -20, color: '#334155', lineWidth: 1, lineStyle: LineStyle.Dotted });
        series.createPriceLine({ price: -80, color: '#334155', lineWidth: 1, lineStyle: LineStyle.Dotted });
        series.createPriceLine({ price: -50, color: '#334155', lineWidth: 1, lineStyle: LineStyle.Dotted });
        registerChart(chart);
        ...
        if (onScreenshotReady) {
            onScreenshotReady(() => {
                ...
                link.download = `${pairKey}_${timeframe}s_williamsr.png`;
        ...
        (async () => {
            ...
            try {
                ...
                console.error('Error bootstrapping Williams %R history:', err);
            }
        })();
    ...
    $effect(() => {
        ...
        const v = iRaw((snap.indicators ?? {}) as IndicatorMap, 'williams_r');
