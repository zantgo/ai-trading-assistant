<script lang="ts">
    ...
    onMount(() => {
        ...
        series = chart.addSeries(LineSeries, { color: '#26a69a', lineWidth: 1, priceLineVisible: false });
        series.createPriceLine({ price: 50, color: '#334155', lineWidth: 1, lineStyle: LineStyle.Solid });
        series.createPriceLine({ price: -50, color: '#334155', lineWidth: 1, lineStyle: LineStyle.Solid });
        registerChart(chart);
        ...
        (async () => {
            ...
            try {
                ...
                if (ih && ih.aroon && ih.aroon.length > 0) {
    ...
    $effect(() => {
        ...
        const v = iRaw((snap.indicators ?? {}) as IndicatorMap, 'aroon');
