<script lang="ts">
    ...
    onMount(() => {
        ...
        series = chart.addSeries(LineSeries, { color: '#ce93d8', lineWidth: 1, priceLineVisible: false });
        ...
        (async () => {
            ...
            try {
                ...
                if (ih && ih.force_index && ih.force_index.length > 0) {
                    const seen = new Set<number>();
                    const d: { time: Time; value: number }[] = [];
                    for (let i = 0; i < ih.times.length; i++) {
                        const t = ih.times[i];
                        const v = ih.force_index[i];
                ...
                console.error('Error bootstrapping Force Index history:', err);
            }
        })();
    ...
    $effect(() => {
        ...
        const v = iRaw((snap.indicators ?? {}) as IndicatorMap, 'force_index');
