<script lang="ts">
    ...
    onMount(() => {
        ...
        series = chart.addSeries(LineSeries, { color: '#4dd0e1', lineWidth: 1, priceLineVisible: false });
        series.createPriceLine({ price: 0, color: '#334155', lineWidth: 1, lineStyle: LineStyle.Dotted });
        registerChart(chart);
        ...
        if (onScreenshotReady) {
            onScreenshotReady(() => {
                ...
                link.download = `${pairKey}_${timeframe}s_ao.png`;
        ...
        (async () => {
            ...
            try {
                ...
                if (ih && ih.awesome_oscillator && ih.awesome_oscillator.length > 0) {
                    const seen = new Set<number>();
                    const d: { time: Time; value: number }[] = [];
                    for (let i = 0; i < ih.times.length; i++) {
                        const t = ih.times[i];
                        const v = ih.awesome_oscillator[i];
                ...
                console.error('Error bootstrapping AO history:', err);
            }
        })();
    ...
    $effect(() => {
        ...
        const v = iRaw((snap.indicators ?? {}) as IndicatorMap, 'awesome_oscillator');
