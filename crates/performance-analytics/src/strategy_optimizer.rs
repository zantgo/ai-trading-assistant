...
pub async fn run_strategy_optimizer(cfg: OptimizerConfig) {
    ...
    loop {
        ...
        let report = build_optimization_report(&trades);
...
/// Shared regime-optimization computation: groups closed trades by market
/// regime, computes win rate / profit factor / avg R-multiple / total PnL per
/// regime, and derives recommendation strings. Used by the scheduled
/// `run_strategy_optimizer` and by `GET /api/analytics/optimization`
/// (single source of truth — the old inline copy in the handler was removed).
pub fn build_optimization_report(
    trades: &[database_storage::ClosedTradeRow],
