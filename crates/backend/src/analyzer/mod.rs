        ...
        match event {
            NormalizedEvent::Trade(ref trade) => {
                ...
                if let Some(completed) = completed_opt.filter(|c| c.start_time_ms > t_last_hist) {
                    ...
                    // Active position context: no paper trading, always zero.
                    ...
                    // Paper-trading position invalidation removed (no paper trading).
                    // Market signals (CHoCH, decisive close, etc.) continue to
                    // be computed for monitoring only.
