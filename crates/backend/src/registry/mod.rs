...
use crate::workspace::{ConfigState, Workspace, WorkspaceStatus};
...
pub async fn add_workspace(
    ...
    // Check for duplicate
    {
        let instances = state.workspaces.read().await;
    ...
    let pair_cfg = config_guard.workspaces.get(&pair_key);
...
pub async fn pause_workspace(state: &Arc<AppState>, workspace_id: &str) -> Result<(), String> {
    let instances = state.workspaces.read().await;
    let instance = instances
        .values()
        .find(|i| i.id == workspace_id)
        .cloned()
        .ok_or_else(|| format!("Workspace {} not found", workspace_id))?;
    drop(instances);

    let mut config_state = instance.config_state.write().await;
    if config_state.status == WorkspaceStatus::Stopped {
        return Err("Cannot pause a stopped instance".to_string());
    }
    config_state.status = WorkspaceStatus::Paused;
    println!(
        "⏸️  Instance paused: {} ({})",
...
pub async fn stop_workspace(state: &Arc<AppState>, workspace_id: &str) -> Result<(), String> {
    let instances = state.workspaces.read().await;
    let instance = instances
        .values()
        .find(|i| i.id == workspace_id)
    ...
    instance.config_state.write().await.status = WorkspaceStatus::Stopped;
...
pub async fn delete_workspace(state: &Arc<AppState>, workspace_id: &str) -> Result<(), String> {
    let (pair_key, instance) = {
        let instances = state.workspaces.read().await;
        let (pk, inst) = instances
            .iter()
            .find(|(_, i)| i.id == workspace_id)
    ...
    {
        let is_stopped = instance.config_state.read().await.status == WorkspaceStatus::Stopped;
        if !is_stopped {
            stop_workspace(state, workspace_id).await?;
        }
    }

    state.workspaces.write().await.remove(&pair_key);

    {
        ...
        config.workspaces.remove(&pair_key);
...
pub async fn recharge_instance(
    ...
    let new_instance = Arc::new(Workspace {
...
pub async fn list_instances(state: &Arc<AppState>) -> Vec<WorkspaceSummary> {
    let instances = state.workspaces.read().await;
