...
use crate::server::types::{
    AddWorkspaceRequest, WorkspaceConfigPayload, WorkspaceDetailQuery,
    WorkspaceIntervalsRequest, WorkspaceListResponse,
};
...
pub async fn serve_list_workspaces(
    State(state): State<Arc<AppState>>,
    Query(query): Query<WorkspaceDetailQuery>,
) -> impl IntoResponse {
    let all_summaries = registry::list_instances(&state).await;
...
pub async fn serve_add_workspace(
    State(state): State<Arc<AppState>>,
    Json(payload): Json<AddWorkspaceRequest>,
) -> impl IntoResponse {
    ...
    match registry::add_workspace(&state, (base, quote))
        .await
    {
        Ok(instance) => (
            ...
            Json(serde_json::json!({
                ...
                "message": format!("Workspace {} created", instance.pair_display()),
            })),
        )
            .into_response(),
        Err(e) => (
...
pub async fn serve_delete_workspace(
    State(state): State<Arc<AppState>>,
    Path(workspace_id): Path<String>,
) -> impl IntoResponse {
    match registry::delete_workspace(&state, &workspace_id).await {
        Ok(()) => (
...
pub async fn serve_delete_workspace_by_pair(
    State(state): State<Arc<AppState>>,
    ...
    match workspace_id {
        Some(id) => match registry::delete_workspace(&state, &id).await {
            Ok(()) => (
                axum::http::StatusCode::OK,
                format!("Workspace {} deleted", pair_key),
            )
                .into_response(),
            Err(e) => (axum::http::StatusCode::NOT_FOUND, e).into_response(),
        },
...
pub async fn serve_get_workspace_detail(
    State(state): State<Arc<AppState>>,
    Path(workspace_id): Path<String>,
) -> impl IntoResponse {
    let instance = state.get_workspace_by_id(&workspace_id).await;
...
pub async fn serve_update_workspace_config(
    State(state): State<Arc<AppState>>,
    Path(workspace_id): Path<String>,
    Json(payload): Json<WorkspaceConfigPayload>,
) -> impl IntoResponse {
    let pair_key = state
    ...
    match pair_key {
        Some(pk) => {
            ...
            let existing = config.workspaces.get(&pk).cloned().unwrap_or_else(|| {
                let default_indicators = crate::config::IndicatorsConfig::default();
                crate::config::WorkspaceSpecificConfig {
            ...
            let specific_config = crate::config::WorkspaceSpecificConfig {
            ...
            config.workspaces.insert(pk.clone(), specific_config);
            crate::config::save_workspaces(&config.workspaces).await;
            drop(config);
...
pub async fn serve_pause_workspace(
    State(state): State<Arc<AppState>>,
    Path(workspace_id): Path<String>,
) -> impl IntoResponse {
    match registry::pause_workspace(&state, &workspace_id).await {
        Ok(()) => (
            axum::http::StatusCode::OK,
            format!("Workspace {} paused", workspace_id),
        )
            .into_response(),
...
pub async fn serve_stop_workspace(
    State(state): State<Arc<AppState>>,
    Path(workspace_id): Path<String>,
) -> impl IntoResponse {
    match registry::stop_workspace(&state, &workspace_id).await {
        Ok(()) => (
            axum::http::StatusCode::OK,
            format!("Workspace {} stopped", workspace_id),
        )
            .into_response(),
        Err(e) => (axum::http::StatusCode::BAD_REQUEST, e).into_response(),
    }
...
pub async fn serve_reset_safety(
    ...
    match instance {
        Some(inst) => {
            ...
            (
                axum::http::StatusCode::OK,
                format!("Safety counter reset for instance {}", workspace_id),
            )
                .into_response()
        }
        None => (axum::http::StatusCode::NOT_FOUND, "Instance not found").into_response(),
...
pub async fn serve_workspace_intervals(
    State(state): State<Arc<AppState>>,
    Path(workspace_id): Path<String>,
    Json(payload): Json<WorkspaceIntervalsRequest>,
) -> impl IntoResponse {
    let instance = state.get_workspace_by_id(&workspace_id).await;
