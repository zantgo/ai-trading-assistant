use backend::analyzer::{ActivePair, TimeframePipeline};
use backend::config::AppConfig;
use backend::config::FibonacciConfig;
use backend::db;
use backend::server::{self, AppState};
use backend::workspace::TimeframeBuffers;
use backend::sr_engine::SrRoleTracker;
...
async fn test_websocket_stream_with_active_pair() {
    ...
    let instance = Arc::new(engine::workspace::Workspace::new(
        "inst_test".to_string(),
