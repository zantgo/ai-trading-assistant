...
use shared::normalized::{
    CandleGenerator, Exchange, NormalizedEvent, NormalizedTrade, SymbolMapper, TradeSide,
};
...
fn test_normalized_event_status_serialization() {
    let event = NormalizedEvent::Status {
        exchange: Exchange::Hyperliquid,
        status: shared::normalized::ConnectionStatus::Connected,
    ...
    match parsed {
        NormalizedEvent::Status {
            ...
            assert_eq!(status, shared::normalized::ConnectionStatus::Connected);
