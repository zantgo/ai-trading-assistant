...
/// Bridge: build a [`SnapshotValues`] from an already-flattened indicator
/// map + current price. Callers (api-gateway handlers) extract the
/// normalized map and current_price from their HTTP DTOs and pass them in
/// directly — `portfolio-supervisor` does not need to know the shape of
/// `IndicatorSnapshot` or any HTTP request struct.
...
/// Flat-field input bundle for `snapshot_values_from_evaluate`. Lets the
/// HTTP layer construct the inputs from its `EvaluateRequest` DTO and hand
/// them to `portfolio-supervisor` without crossing the crate boundary.
...
/// [`EvaluateInputs`] bundle (frontend decision-profile evaluator).
pub fn snapshot_values_from_evaluate(req: &EvaluateInputs) -> SnapshotValues {
    use market_analyzer::indicators::squeeze::MomentumDirection;
    use market_analyzer::indicators::IndicatorInputs;
...
pub async fn evaluate_profile(
    ...
    let profiles = database_storage::decision_profiles_list(pool).await;
