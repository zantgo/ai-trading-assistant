...
pub struct NormalizeParams<'a> {
    ...
    pub anchored_vwap: Option<crate::indicators::AvwapOutput>,
    ...
    pub candlestick: Option<crate::indicators::CandlestickResult>,
    pub candlestick_min_confidence: f64,
    /// Ichimoku Cloud output for this bar.
    pub ichimoku: Option<crate::indicators::IchimokuOutput>,
    /// CCI reading for this bar.
    pub cci: Option<Decimal>,
    /// PSAR reading for this bar.
    pub psar: Option<crate::indicators::PsarOutput>,
    ...
    pub stddev_channel: Option<crate::indicators::SdChannelOutput>,
    /// Volume Profile reading for this bar.
    pub volume_profile: Option<crate::indicators::VolumeProfileOutput>,
...
pub fn build_indicator_map(
    ...
    let inputs = IndicatorInputs {
        ...
        smc_structure_bullish: p
            .smc
            .as_ref()
            .map(|s| matches!(s.structure, crate::indicators::MarketStructure::Bullish))
            .unwrap_or(false),
        smc_structure_bearish: p
            .smc
            .as_ref()
            .map(|s| matches!(s.structure, crate::indicators::MarketStructure::Bearish))
            .unwrap_or(false),
    ...
    inject_sharpe_ratio(&mut map, p.price_trend_sharpe, bar_count, shadow);
...
/// Inject the L1 `price_trend_sharpe` (v6.11). Gated on the canonical
/// 300-bar window (equal to `[candle_buffer] size` — the indicator goes
/// `Live` exactly when the pipeline buffer fills). Injection runs after
/// the `bars_required` gate and before the CA-06 retain, so a disabled
