...
async fn test_primary_to_backup_switch_on_failure() {
    ...
    assert_eq!(
        active,
        engine::api_failover::KeySource::Backup,
...
async fn test_permanent_halt_blocks_subsequent_calls() {
    ...
    for _ in 0..3 {
        let result = state
            .execute_with_failover("test", |_| async { Err::<&str, _>("always fails".into()) })
            .await;
    ...
    assert!(
        state.permanently_failed.load(Ordering::Relaxed),
    ...
    let final_result = state
        .execute_with_failover("test", |_| async {
            Ok::<&str, _>("should never be called")
        })
        .await;
    assert!(final_result.is_err());
    assert!(
        final_result.unwrap_err().contains("cooling down"),
        "Should return cooling down error"
    );
}

#[tokio::test]
async fn test_success_resets_consecutive_failures() {
    let state = ApiFailoverState::new(Some("good-key".into()), None, 0, 1, 10, 0);
    ...
    for _ in 0..2 {
        ...
        let result = state
            .execute_with_failover("test", move |_| {
                count.fetch_add(1, Ordering::Relaxed);
                async move { Err::<&str, _>("fail".into()) }
            })
            .await;
        assert!(result.is_err());
    }
