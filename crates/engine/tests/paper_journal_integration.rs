...
async fn test_full_paper_trade_to_journal_loop() {
    ...
    let open = engine::paper_trading::scale_in_portion(
        &pool, &tx, "BTC", "LONG", 50000.0, 1, 48000.0,
    ).await;
    ...
    assert!(balance_after_open.current_cash < 10000.0,
        "Cash should be deducted after open, got {}", balance_after_open.current_cash);

    // ── STEP 2: Scale-in portion 2 ────────────────────────────────────
    let p2 = engine::paper_trading::scale_in_portion(
        &pool, &tx, "BTC", "LONG", 51000.0, 2, 48500.0,
    ).await;
    ...
    let p3 = engine::paper_trading::scale_in_portion(
        &pool, &tx, "BTC", "LONG", 52000.0, 3, 49000.0,
    ).await;
    ...
    assert!(pos_after_close.is_none(), "Active position should be deleted after close");

    // ── VERIFY: paper_trades recorded ─────────────────────────────────
    let trade_count: (i64,) = sqlx::query_as(
        "SELECT COUNT(*) FROM paper_trades WHERE symbol = 'BTC'"
    )
    .fetch_one(&pool)
    .await
    .unwrap();
    assert!(trade_count.0 >= 1, "paper_trades should record the closed trade, got {}", trade_count.0);
    ...
    assert!(final_balance.current_cash > 0.0,
        "Balance should still be positive, got {}", final_balance.current_cash);

    // ── VERIFY: trade_learning_journal populated ──────────────────────
    let journal_count: (i64,) = sqlx::query_as(
        "SELECT COUNT(*) FROM trade_learning_journal WHERE asset = 'BTC'"
    )
    .fetch_one(&pool)
    .await
    .unwrap();
    assert!(journal_count.0 >= 1,
        "trade_learning_journal should have a record, got {}", journal_count.0);
}
...
async fn test_close_loss_preserves_journal_with_negative_pnl() {
    ...
    let open = engine::paper_trading::verify_margin_and_open_with_alloc(
        &pool, &tx, "ETH", "LONG", 3000.0, Some(20.0),
    ).await;
    assert!(open.success);
    ...
    .await
    .unwrap();
    assert!(trade.0 < 0.0, "PnL should be negative for a loss, got {}", trade.0);

    let balance = db::paper_get_balance(&pool, "ETH").await;
    assert!(balance.current_cash < 10000.0,
        "Cash should decrease after a loss, got {}", balance.current_cash);
}
