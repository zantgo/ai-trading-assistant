...
async fn test_load_max_pairs_stability() {
    tokio::time::timeout(tokio::time::Duration::from_secs(60), async {
        ...
        for pair_idx in 0..num_pairs {
            ...
            tokio::spawn(async move {
                analyzer::run_single(mid_rx, m_t, mid_broadcast, mid_tf, mfib, m_div, mh, Arc::new(RwLock::new(None)), Arc::new(RwLock::new(VecDeque::new())), m_sym, m_pk, 60, "Micro", m_cancel, None, None).await;
            });
