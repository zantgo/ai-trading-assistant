...
async fn test_load_max_pairs_stability() {
    tokio::time::timeout(tokio::time::Duration::from_secs(60), async {
        ...
        for pair_idx in 0..num_pairs {
            ...
            let mid_history = Arc::new(RwLock::new(VecDeque::<NormalizedCandle>::with_capacity(
                cap,
            )));
            ...
            for trade_idx in 0..trades_per_pair {
                for pair_idx in 0..num_pairs {
                    ...
                    let trade = NormalizedTrade {
                        ...
                        price: rust_decimal::Decimal::from_f64_retain(price)
                            .unwrap_or(dec!(50000.00)),
                        size: dec!(0.5),
                        side: if trade_idx % 2 == 0 {
                            TradeSide::Buy
                        } else {
                            TradeSide::Sell
                        },
        ...
        eprintln!(
            "Load test: {} pairs, {} total candles",
            num_pairs, total_candles
        );
        assert!(
            total_candles > 0,
            "At least some candles should be produced"
        );
