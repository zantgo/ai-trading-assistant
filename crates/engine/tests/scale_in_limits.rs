...
async fn test_fourth_scale_in_rejected() {
    ...
    let p1 = engine::paper_trading::scale_in_portion(
        &pool, &tx, "BTC", "LONG", 50000.0, 1, 49000.0,
    ).await;
    ...
    let p2 = engine::paper_trading::scale_in_portion(
        &pool, &tx, "BTC", "LONG", 51000.0, 2, 49500.0,
    ).await;
    ...
    let p3 = engine::paper_trading::scale_in_portion(
        &pool, &tx, "BTC", "LONG", 52000.0, 3, 50000.0,
    ).await;
    ...
    let p4 = engine::paper_trading::scale_in_portion(
        &pool, &tx, "BTC", "LONG", 53000.0, 4, 50500.0,
    ).await;
...
async fn test_wrong_direction_scale_in_rejected() {
    ...
    let open = engine::paper_trading::scale_in_portion(
        &pool, &tx, "ETH", "LONG", 3000.0, 1, 2900.0,
    ).await;
    assert!(open.success, "LONG open should succeed");

    tokio::time::sleep(tokio::time::Duration::from_millis(100)).await;

    let wrong = engine::paper_trading::scale_in_portion(
        &pool, &tx, "ETH", "SHORT", 3100.0, 2, 3200.0,
    ).await;
    assert!(!wrong.success, "SHORT scale-in on LONG position should be rejected");
    assert!(wrong.message.contains("Cannot"), 
        "Rejection should mention direction mismatch, got: {}", wrong.message);
}
