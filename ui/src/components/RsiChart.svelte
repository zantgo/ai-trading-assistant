<script lang="ts">
    ...
    function screenshotChart() { if (chart) takeChartScreenshot(chart, `rsi-${pairKey}-${formatTimeframeLabel(timeframe)}`); }
    ...
    onMount(() => {
        ...
        if (onScreenshotReady) {
            onScreenshotReady(() => {
                ...
                link.download = `${pairKey}_${formatTimeframeLabel(timeframe)}_rsi.png`;
                link.href = dataUrl;
                link.click();
            });
        }
