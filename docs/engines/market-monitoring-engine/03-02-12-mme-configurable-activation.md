...
| CA-10 | Policy guardrail: a TAE policy whose conditions reference a disabled indicator/signal is **rejected at save time** (`409`) listing references. On config change, already-active affected policies transition to **`AUTO_PAUSED`** (policy-scope state; see `03-03-04-tae-execution-policy-spec.md` §2.3) with an audit event and operator acknowledgment to resume. |
