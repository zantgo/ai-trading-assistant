...
> **Five-regime canonical form (SIG-14 — alignment with §6).** A previous version of this table defined only four regimes (`Congestion < 20`, `Emerging 20-25`, `Strong 25-40`, `Extreme > 40`), omitting the `Transition [18, 20)` zone. That omission contradicted §6's piecewise normalization which already carries the `[18, 20)` smooth-ramp band. The canonical form above restores parity: every band in §6 has a matching regime label here.
