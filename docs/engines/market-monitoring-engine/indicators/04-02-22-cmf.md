...
**Divergence Lifecycle:** Potential (oscillator disagrees with price, no structural break) → Confirmed (candle close breaks nearest S/R boundary by >0.2% tolerance). The divergence is emitted as a nested `Divergence` signal on the `cmf` key — there is no separate `cmf_divergence` registry entry or JSON key.
