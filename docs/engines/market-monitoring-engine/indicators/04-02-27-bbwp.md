...
> **Confidence redundancy.** The `+0.15` signal boost is meaningful only in the mid-bands (where the base confidence is below the ceiling); it would saturate at `1.0` together with the base confidence at the extremes (`BBWP < 10` and `BBWP > 90`), making the boost a no-op there. The canonical mapping skips the redundant boost at the saturation bands.
