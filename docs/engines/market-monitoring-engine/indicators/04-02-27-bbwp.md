...
confidence = {
  BBWP < 10              → 0.85 + 0.15     # extreme compression — high reliability for a breakout watch
  10 ≤ BBWP < 25         → 0.60            # compression zone
  25 ≤ BBWP ≤ 75         → 0.50            # neutral volatility territory
  75 < BBWP ≤ 90         → 0.60            # expansion zone
  BBWP > 90              → 0.85 + 0.15     # extreme exhaustion — high reliability for reversion warning
}
signal_boost = +0.15 if any BBWP signal is Active
```
