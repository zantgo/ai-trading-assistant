...
|-----------|--------------|------------------|-----------|
| VolatilityCycle | MAX_VOLATILITY_COMPRESSION | **BBWP < 10** (extreme compression — coiled energy before expansion) | Neutral |
...
> **Confidence redundancy correction (SIG-18 — canonical simplification).** A previous version of this table wrote `BBWP < 10 → 0.85 + 0.15` and `BBWP > 90 → 0.85 + 0.15` directly into the band mapping — which *baked in* `1.0` as the base confidence at the extremes. The `+0.15` signal boost was then added on top, and the clamp brought it back to `1.0`. This is redundant: the boost never contributes at the extremes because the band mapping already saturates the ceiling.
...
>
> This makes the `signal_boost` meaningful at every band, not just the inner ones, while preserving the schema invariant `[0.0, 1.0]` enforced at the seam in `crates/shared/src/indicators/normalized/mod.rs::with_confidence`.
