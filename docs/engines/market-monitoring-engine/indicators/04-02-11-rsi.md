...
> **Label mapping (AUDIT-AIU-090).** The runtime emits the bias Threshold via the normalizer's state labels `BULLISH_DISCOUNT` / `BEARISH_PREMIUM` / `EQUILIBRIUM` (the deriver maps them to Threshold signals); the names above are the canonical event names carried in the signal label. The `DIVERGENCE_BULLISH/BEARISH_CONFIRMED` labels replace the previous `OVERSOLD_ACCUMULATION`/`OVERBOUGHT_DISTRIBUTION` override labels on confirmed-divergence bars so a confirmed divergence at RSI=80 no longer fires a spurious OVERSOLD threshold (AUDIT-AIU-033).
