...
| VolatilityCycle | `ATR_CONTRACTING` | Regime = Contracting (`current_ATR / 5p-mean_ATR < 0.98`) — coiled energy before expansion | Neutral |
...
- `atr_regime` — the qualitative regime bucket (`Expanding` | `Contracting` | `Stable`) computed by `classify_regime` in `crates/shared/src/indicators/atr.rs`
