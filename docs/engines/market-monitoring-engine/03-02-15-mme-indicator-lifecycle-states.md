...
| **ILS-06** | **`LOADING → FAILED`** transition triggers when either (a) a calculator panic / `Err` is observed, or (b) `(now_ms - last_updated_at) > stale_threshold_secs` AND `bars_seen < bars_required`. Both sub-conditions are visible in `last_error`. |
...
| **ILS-12** | An indicator is **considered disabled** (absent from the active set per [03-02-12](03-02-12-mme-configurable-activation.md)) — disabled indicators are **not** emitted in `indicator_lifecycle` at all. The lifecycle map only contains entries for active-set indicators, mirroring the existing `indicators` map contract. |
...
| Indicator group | Examples | `bars_required` |
|-----------------|----------|----------------:|
| Trend EMAs | `ema_stack`, `hull_ma` | max slow-EMA period (default 200) |
| Trend others | `supertrend`, `donchian`, `keltner`, `vwap`, `ichimoku`, `psar` | 50 (or specific: ichimoku = 52) |
| Momentum simple | `rsi`, `williams_r`, `cci` | 14 (or 20 for cci) |
| Momentum complex | `stochastic`, `chandemo`, `macd`, `awesome_oscillator` | 26 (slow period) for macd; 14 for stochastic/chandemo; 34 for ao |
| Volume | `volume`, `rvol`, `obv`, `cmf`, `mfi`, `force_index`, `volume_profile` | 20 (rolling) or 50 (volume profile window default) |
| Volatility | `atr`, `bollinger`, `bbwp`, `squeeze`, `hv`, `stddev_channel` | 20 (or specific period per indicator) |
| Structure | `fibonacci`, `support_resistance`, `pivot_points`, `patterns`, `candlestick` | 50 |
| Regime | `aroon`, `choppiness`, `linreg_slope`, `zscore` | 25 (aroon period) or 14 (zscore) |
...
| 3 | LOADING | FAILED | (a) calculator panic / `Err` (b) `now_ms - last_updated_at > stale_threshold_secs` AND `bars_seen < bars_required` (ILS-06) | UI badge red; `last_error` populated |
...
| All 50 indicators `LIVE`, parent pipeline `LIVE` | Pipeline → `LIVE` (DCP-04). |
| All 50 indicators `LOADING`, parent pipeline `LOADING` | Pipeline → `LOADING` (DCP-04). |
...
- `AUDIT-V7-331` — `market-analyzer/registry`: add `bars_required: u32` to each of the 50 indicator metadata entries in `crates/market-analyzer/src/indicators/registry.rs`.
