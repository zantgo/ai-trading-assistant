...
| **DCP-04** | `LIVE` is entered when the buffer has reached `candle_buffer.size` **and** the parent `ConnectionStatus` is `Connected` (DCP-09). All 52 indicators in `indicator_lifecycle` must be either `Live` or `Stale`; if any indicator is `Loading` or `Failed` the pipeline cannot be `Live`. |
