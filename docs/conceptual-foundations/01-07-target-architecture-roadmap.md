...
| **WASM per-instance connection-quality scoring** | Not started. Tracker is process-wide; v6.0 makes it per-(pair, timeframe). | Move tracker to a WASM module to isolate per-instance memory; profile overhead. | Deferred (v4.1 per `AUDIT-V4-078`) | DIE team |
