...
| `market-analyzer` | MME L1–L7 (logical ownership); DIE L2–L4 (physical execution) | 51 indicators across 4 timeframes, signal detection, multi-TF pipeline orchestrator (`ActivePair`, `TimeframePipeline`), `MarketContext` synthesis, candle generation, quality validation, distribution channels, indicator DTO re-exports. |
