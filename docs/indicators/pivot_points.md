...
* **Persistence:** The seven levels flow automatically through the `auxiliary_normalized_data` JSON blob — no schema migration required.
...
[pivot_points]
enabled = true
method = "classic"            # forward-compatible: fibonacci | camarilla | woodie (not yet implemented)
