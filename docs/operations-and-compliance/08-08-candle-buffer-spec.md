...
| **CB-09** | A ≥ 1 minute cold start always completes with **all 50 indicators in `IndicatorLifecycleState::Live`** (every indicator's `bars_required ≤ size = 500`). The user sees a chart with full history on first paint; the only visible loading state is the brief exchange-REST round-trip. |
...
|-----------------:|-----------------------------:|---------------------|-----------------|-------------------|
| `< 60` (sub-min) | 0                            | `LOADING`           | After `size × timeframe_secs` of wall-clock | None (CB-05) |
